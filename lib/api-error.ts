...
export function handleSmbError(error: any, res: NextApiResponse) {
    ...
    if (errCode === 'STATUS_LOGON_FAILURE' || errMessage.includes('LOGON_FAILURE')) {
        return res.status(401).json({ error: 'Authentication failed. Please check your credentials.' });
    }

    if (errCode === 'STATUS_ACCESS_DENIED' || errMessage.includes('ACCESS_DENIED')) {
        return res.status(403).json({ error: 'Permission denied. Ensure the file is not locked or protected.' });
    }
